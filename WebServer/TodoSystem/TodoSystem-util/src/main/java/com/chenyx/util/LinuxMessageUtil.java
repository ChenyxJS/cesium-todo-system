...
public class LinuxMessageUtil {
    ...
    public static String cpuUsage() {
        try {
            Map<?,?> map1 = LinuxMessageUtil.cpuInfo();
            Thread.sleep(5 * 1000);
            Map<?,?> map2 = LinuxMessageUtil.cpuInfo();
            ...
            return  String.format("%.2f", cpusage);
...
//            long memAvailable = Long.parseLong(map.get("MemAvailable").toString());

            double usage = (double) (memused - buffers - cached) / memTotal * 100;
//            double usage = (1-(memAvailable/memTotal)) *100;
...
//                    log.info(line);
                    ...
                    if(temp[0].substring(5).length() > 0 && !temp[0].substring(5).equals("")) {
...
//            log.error(sw.toString());
        ...
        return String.format("%.2f", netUsage);
    }
}
