...
public class SessionUtil {
    ...
    public static void setLoginInfoToSession(HttpSession session, UserInfo userInfo) {
        setLoginUser(userInfo,session);
    }
...
//        }
        ...
        String key = LoginEnum.LOGIN_PERMISSION.getCode()+"-"+role.getRoleId()+"-"+session.getId();
...
//        }
        ...
        String urls = redisCache.getCacheObject(LoginEnum.LOGIN_PERMISSION.getCode()+"-"+role.getRoleId()+"-"+session.getId()+"-urls", String.class);
...
//        }
        ...
        String urls = redisCache.getCacheObject(LoginEnum.LOGIN_MINI_PERMISSION.getCode()+"-"+role.getRoleId()+"-"+session.getId()+"-urls", String.class);
        if(StringUtil.isEmpty(urls)){
            jsonResult.buildFalseNew(RequestEnum.REQUEST_ERROR_NO_LOGIN,"无权限，请尝试重新登陆");
            return jsonResult;
        }
