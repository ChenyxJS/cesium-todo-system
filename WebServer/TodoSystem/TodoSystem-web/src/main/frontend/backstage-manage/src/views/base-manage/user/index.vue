  ...
  <div class="list-view-layout">
    <json-filter :code="titleCode">
      <el-form :inline="true">
        ...
        <el-form-item label="用户名称">
          <el-input v-model="param.userRealNameIsLike" placeholder="请输入" clearable/>
        </el-form-item>

        <el-form-item>
          <el-button @click="doSearch()">查询</el-button>
          <permission-filter code="baseUserAdd" type="primary" @click.native="doAdd()"/>
        </el-form-item>
    ...
    <json-table ref="myJsonTable" :table-heads="tableHeads" :loading="listLoading" :table-data="tableData"
      ...
      <template #operations="iData">
        <permission-filter code="baseUserEdit" size="mini" type="warning"
                           @click.native="doEdit(iData.rowData.scope.row)"/>
        <permission-filter code="baseUserDelete" size="mini" type="danger"
  ...
  data() {
    return {
      ...
      tableHeads: [
        ...
        {
          dataType: "tag",
          tagLabelMap:{  0: "未知", 1: "男", 2: "女" },
          tagTypeMap:{ 0: "info",1: "success",2: "danger" },
          prop: 'userSex',
          label: '性别'
        },
        {
    ...
    doDelete(e) {
      this.$confirm('确定删除？', '提示', {
        ...
        beforeClose: (action, instance, done) => {
          ...
          deleteUser({id: e.userId}).then(result => {
            if (result.success) {
