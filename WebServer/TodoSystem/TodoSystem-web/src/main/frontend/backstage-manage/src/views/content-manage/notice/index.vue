  ...
  <div class="list-view-layout">
    <json-filter :code="titleCode">
      <el-form :inline="true">
        ...
        <el-form-item>
          ...
          <permission-filter code="contentNoticeAdd" type="primary" @click.native="doAdd()"/>
        </el-form-item>
      </el-form>
    </json-filter>
    ...
    <json-table ref="myJsonTable" :table-heads="tableHeads" :loading="listLoading" :table-data="tableData" :page="page"
      ...
      <template #operations="iData">
        <permission-filter code="contentNoticeDelete" size="mini" type="danger"
...
<script>
  ...
  import {getNoticeList, deleteNotice} from "@/api/content-notice"
  ...
  export default {
    ...
    data() {
      return {
        titleCode: 'contentNotice',
        tableHeads: [
          {
            prop: 'noticeClubName',
            label: '社团名称',
            overflowTooltip: 'show'
          },
          {
            prop: 'noticeTitle',
            label: '通知标题',
      ...
      doDelete(e) {
        this.$confirm('确定删除？', '提示', {
          ...
          beforeClose: (action, instance, done) => {
            ...
            deleteNotice({id: e.noticeId}).then(result => {
              if (result.success) {
