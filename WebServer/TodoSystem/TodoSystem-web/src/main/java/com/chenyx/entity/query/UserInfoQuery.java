...
public class UserInfoQuery extends QueryParam {
    ...
    public QueryWrapper<UserInfo> buildExample() {
        ...
        // 身份证
        if (StringUtil.isNotEmpty(getParam().getUserIdNumber())) {
            query.eq("user_id_number", getParam().getUserIdNumber());
            update.eq("user_id_number", getParam().getUserIdNumber());
        }
        ...
        // 国家
        if (StringUtil.isNotEmpty(getParam().getUserCountry())) {
            query.eq("user_country", getParam().getUserCountry());
            update.eq("user_country", getParam().getUserCountry());
        }
        ...
        // 省份
        if (StringUtil.isNotEmpty(getParam().getUserProvince())) {
            query.eq("user_province", getParam().getUserProvince());
            update.eq("user_province", getParam().getUserProvince());
        }
        ...
        // 城市
        if (StringUtil.isNotEmpty(getParam().getUserCity())) {
            query.eq("user_city", getParam().getUserCity());
            update.eq("user_city", getParam().getUserCity());
        }
        ...
        // 县区
        if (StringUtil.isNotEmpty(getParam().getUserArea())) {
            query.eq("user_area", getParam().getUserArea());
            update.eq("user_area", getParam().getUserArea());
        }
        if (StringUtil.isNotEmpty(getUserAreaIsLike())) {
            query.like("user_area", getUserAreaIsLike());
            update.like("user_area", getUserAreaIsLike());
        }

        // 城镇
        if (StringUtil.isNotEmpty(getParam().getUserTown())) {
            query.eq("user_town", getParam().getUserTown());
            update.eq("user_town", getParam().getUserTown());
        }
        ...
        // 详细地址
        if (StringUtil.isNotEmpty(getParam().getUserAddress())) {
            query.eq("user_address", getParam().getUserAddress());
            update.eq("user_address", getParam().getUserAddress());
        }
