<template>
  <div class="list-view-table">
    <div class="json-table-view">
      ...
      >
        ...
        <el-table-column
          ...
          :type="'index'"
          :fixed="tableType === 'selection' ? 'left' : ''"
          :label="tableType === 'index' ? '#' : ''"
          header-align="center"
          width="50"
        />
        ...
        >
          <!--     在tableHeads的列上加上：（只能选择其中一个，不加则直接显示返回数据）
                  ...
                  dataType:url 链接（图片）-->
          <template #default="scope">
            <span v-if="col.dataType === 'date'"></span>
            <span v-else-if="col.dataType === 'percentage'">{{
              Number(scope.row[col.prop] * 100).toFixed(
                col.decimal ? col.decimal : 0
              ) + "%"
            }}</span>
            <el-image
              v-else-if="col.dataType === 'url'"
            ...
            <el-tag
              ...
              :type="
                col.tagTypeMap[scope.row[col.prop]]
                  ? col.tagTypeMap[scope.row[col.prop]]
                  : 'success'
              "
              effect="dark"
            >
              {{
                col.tagLabelMap[scope.row[col.prop]]
                  ? col.tagLabelMap[scope.row[col.prop]]
                  : scope.row[col.prop]
              }}
            </el-tag>
    ...
    <div v-if="page !== null" class="page">
      ...
      >
        <span class="common-pagination__total-pages">共{{}}页</span>
      </el-pagination>
    </div>
  </div>
</template>
